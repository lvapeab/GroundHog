...
class MainLoop(object):
    ...
    def load(self, model_path=None, timings_path=None, exclude_params=[]):
        ...
        try:
            ...
            traceback.print_exc()
        if not(self.state['reload_lm']) or self.state['reload']:
            try:
    ...
    def main(self):
        ...
        #TODO check the inputs for the start function
        if self.valid_data is not None and hasattr(self.valid_data, "start"):
        ...
        while (self.step < self.state['loopIters'] and
            ...
            try:
                ...
                if self.state.has_key('bleu_val_frequency') and \
                    ...
                    # copy parameters from enc_dec to enc_dec_validation
                    nameToIdx = dict([(x.name, idx) for idx, x in
                        enumerate(self.bleu_val_fn.beam_search.enc_dec.params)])
                    for p in self.model.params:
                        self.bleu_val_fn.beam_search.enc_dec.params[nameToIdx[p.name]].set_value(
                                                                                    p.get_value())

                    if self.bleu_val_fn():
                        print "Saving ...", self.state['prefix'] + 'best_bleu_' + 'model.npz'
