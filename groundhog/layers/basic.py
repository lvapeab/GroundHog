...
class Layer(Container):
    ...
    def flatten(self):
       assert hasattr(self, 'out'), 'all layers need a default output'
       new_obj = utils.copy(self)
       new_obj.out = new_obj.out.flatten()
       return new_obj

    def prob_complement(self):
        """
            Find the probabilistic complement of the random variable, e.g. (1 - alpha)
        """
        assert hasattr(self, 'out'), 'all layers need a default output'
        new_obj = utils.copy(self)
        new_obj.out = 1 - new_obj.out
        return new_obj
