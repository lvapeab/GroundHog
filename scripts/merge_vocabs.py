...
'''
Generates an intersection vocabulary pickle file given input text file.
...
Usage:
    generate_intersect_vocab.py -v 30000 train.tags.zh-en.en.tok train.tags.tr-en.en.tok
...
def create_dictionary():
    ...
    if args.vocab is not None:
        if args.vocab <= 2:
            ...
            args.vocab = len(intersect_dict) + 2
        vocab_count = intersect_dict.most_common(args.vocab - 2)
        ...
        for ctr, base_filename in zip(counters, base_filenames):
            logger.info("  %2.2f%% of the file %s" %
                       (100.0 * (1 - (sum((ctr - intersect_dict).values())*1.0 / sum(ctr.values()))),
                       base_filename))
    else:
    ...
    return intersect_dict, sentence_counts, counters, vocab
